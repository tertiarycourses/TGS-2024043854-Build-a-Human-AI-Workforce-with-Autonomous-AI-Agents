...
## 📺 Reference Video
[Hire the Members Under the Hiring Task](https://www.youtube.com/watch?v=tgqcHHxiwfk&list=PLZslvC3k6NibmbMzfuyc3UGXkIC5k6YvC&index=6)
...
- **Reject** with a comment — the CEO revises the proposal (e.g. lower budget, sharper mandate) and resubmits.

Nothing joins the company without an explicit Board decision — this gate is the human-in-the-loop control at the heart of Paperclip governance.
...
| Symptom | Fix |
| --- | --- |
| CEO doesn't start the hiring task | Confirm the task is assigned to the CEO and moved to **in_progress**, and that an adaptor is enabled (Lab 29). |
| Hiring plan is vague or misses roles | The brief drives the plan — check the task description matches Lab 30's core-task text, then reject with a comment asking for the four named roles. |
| No approval prompt appears | Look for pending approvals on the task and in notifications — each proposed hire has its own gate entry. |
| A rejected hire never comes back | Rejection needs feedback — reopen the gate decision and add a comment so the CEO knows what to revise. |
| Specialists hired but idle | The backlog was never assigned — assign the remaining Lab 30 tasks to the new members (step 6). |
