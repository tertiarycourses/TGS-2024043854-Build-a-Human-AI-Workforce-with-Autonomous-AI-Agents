...
prodoc.add_version_control(doc,[
 ...
 ("1.4",C.VERSION_DATE,"Security masterclass slides added to the Hermes Security lab (defense-in-depth layers; the gateway's six-check default-deny trust chain; channel lockdown dials; the three approval modes + the Tirith scanner; a live Dangerous-Command approval; YOLO mode; the unrecoverable-command blocklist floor; prompt-injection scanning of context files). Security lab steps expanded with the channel allowlist and approvals.mode.",C.TRAINER),
])
prodoc.add_toc(doc)
